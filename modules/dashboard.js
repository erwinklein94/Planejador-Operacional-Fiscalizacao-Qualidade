...
export function renderDashboard(ctx) {
  ...
  <div class="grid-2"><section class="panel"><div class="section-title"><div><h2>Cobertura por material</h2><p>Esforço alocado em relação à demanda</p></div><a class="text-link" href="#/cobertura">Ver detalhes ${icon("chevron")}</a></div>${
    s.materials
      .filter((x) => x.active)
      .map((material) => {
  ...
  <section class="panel"><div class="section-title"><div><h2>Distribuição da demanda</h2><p>Semana ${weekNumber(week)} · ${demands.length} demandas</p></div>${badge("fiscal-dias", "neutral")}</div><div class="capacity-summary"><div class="donut"><svg viewBox="0 0 168 168" role="img" aria-label="${m.coverage === null ? "Sem demanda" : pct(m.coverage) + " de cobertura"}"><circle cx="84" cy="84" r="${radius}" fill="none" stroke="#f2ddc9" stroke-width="16"/><circle cx="84" cy="84" r="${radius}" fill="none" stroke="#003865" stroke-width="16" stroke-dasharray="${(circumference * (m.coverage || 0)) / 100} ${circumference}" stroke-linecap="round"/></svg><div class="donut-center"><strong>${num(m.required / s.settings.hoursPerDay)}</strong><small>fiscal-dias de demanda</small></div></div><div class="legend"><div class="legend-row"><i></i><span>Demanda coberta<strong>${num(m.covered / s.settings.hoursPerDay)} <small>fiscal-dias</small></strong></span></div><div class="legend-row"><i></i><span>Demanda não coberta<strong>${num(m.uncovered / s.settings.hoursPerDay)} <small>fiscal-dias</small></strong></span></div></div></div><div class="chart-foot">${icon("clock")} Demanda reprimida prevista: <strong>${m.repressedForecast === null ? "sem demanda" : pct(m.repressedForecast)}</strong></div></section></div>
  <div class="grid-2 spaced"><section class="panel flush"><div class="section-title"><div><h2>Prioridades sem cobertura</h2><p>As 5 demandas que exigem sua atenção primeiro</p></div><a class="text-link" href="#/demandas?coverage=uncovered">Ver todas ${icon("chevron")}</a></div>${demandTable(uncovered, s, week, true)}</section><section class="panel"><div class="section-title"><div><h2>Fornecedores de maior risco</h2><p>Score de risco · quanto maior, maior a atenção</p></div><a class="text-link" href="#/risco">Mapa de risco ${icon("chevron")}</a></div>${suppliers.map((f, i) => `<div class="ranking"><span class="rank-number">${String(i + 1).padStart(2, "0")}</span><span class="supplier-symbol">${icon("factory")}</span><div class="ranking-copy"><button class="table-link" data-action="supplier-detail" data-id="${f.id}"><strong>${esc(f.name)}</strong></button><small>${esc(f.city)} · ${esc(f.uf)}</small></div><div class="risk-score"><strong class="${f.risk.score >= 70 ? "danger-text" : ""}">${f.risk.score}</strong><small>/100</small><div>${badge(f.risk.label, f.risk.tone)}</div></div></div>`).join("") || empty("Nenhum fornecedor")}</section></div>
