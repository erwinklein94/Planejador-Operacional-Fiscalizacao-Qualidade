...
export function renderSettings(ctx) {
  ...
  return `${heading("Configurações", "Parâmetros de planejamento, pesos de risco e gestão dos dados locais.")}<div class="settings-grid"><section class="panel"><h2>Parâmetros da operação</h2><form id="settings-form"><label class="field">Nome do usuário local<input name="userName" required maxlength="80" value="${esc(s.settings.userName)}"></label><div class="form-grid"><label class="field">Jornada de um fiscal-dia (h)<input name="hoursPerDay" type="number" min="1" max="12" step=".5" required value="${s.settings.hoursPerDay}"><small>Alocações são salvas em horas.</small></label><label class="field">Meta de cobertura (%)<input name="coverageTarget" type="number" min="0" max="100" required value="${s.settings.coverageTarget}"></label></div><h3 class="spaced">Pesos do score de prioridade</h3><p class="muted">Os sete componentes devem somar 100 pontos.</p><div class="form-grid">${Object.entries(
    ...
    .join(
      "",
    )}</div><div id="settings-error" role="alert"></div><button class="btn primary spaced" type="submit">Salvar configurações</button></form></section><div class="stack"><section class="panel"><h2>Dados neste navegador</h2><p class="muted spaced">Os registros ficam salvos apenas neste navegador e dispositivo. Exporte backups para conservar os dados ou transferi-los para outro computador.</p><div class="setting-actions">${button("Exportar backup JSON", "export", "", "download")}${button("Importar backup", "import", "", "history")}</div><input type="file" id="import-file" accept=".json,application/json" hidden><div class="notice">${s.demo ? "Base de demonstração ativa. Todos os nomes, ocorrências e indicadores são exemplos." : "Base local de trabalho. Nenhum dado é enviado para um servidor."}</div></section><section class="panel"><h2>Base de demonstração</h2><p class="muted spaced">Carregar os exemplos ou começar com uma base vazia substitui todos os registros locais. Um backup será oferecido antes da confirmação.</p><div class="setting-actions">${button("Carregar demonstração", "reset-demo", "", "history")}${button("Limpar dados locais", "reset-empty", "danger")}</div></section><section class="panel"><h2>Evolução da plataforma</h2><p class="muted spaced">Persistência isolada em um serviço assíncrono. A integração futura poderá adicionar usuários, perfis e sincronização mantendo os módulos de planejamento.</p><div class="notice">Nesta versão, o usuário local pode aprovar alocações e ajustar regimes. Ainda não há autenticação nem controle de acesso por perfil.</div></section></div></div>`;
}
